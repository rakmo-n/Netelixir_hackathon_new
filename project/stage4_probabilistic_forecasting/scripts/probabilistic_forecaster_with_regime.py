"""
Stage 4: Probabilistic Forecasting with Regime Change Detection
===============================================================
Train HistGradientBoostingRegressor at multiple quantiles (0.10, 0.50, 0.90)
with a `regime_shift` binary feature to capture structural breaks in Feb 2026.

For grains with test_mean / train_mean > 2.0, an additional
`regime_shift_interaction` feature (regime_shift * target_spend_Hd) is added
to capture the regime's effect on budget sensitivity.
...
--------
  ...
  Regime detection:
    regime_shift (0 before 2026-02-01, 1 on or after)
    regime_shift_interaction (for affected grains only)
...
OUTPUTS
-------
  outputs/with_regime/probabilistic_forecasts.csv  - every test-date forecast with bands
  outputs/with_regime/validation_metrics.csv       - per-grain/horizon summary
  outputs/with_regime/feature_importances.csv      - top features per grain/horizon/q
  outputs/with_regime/diagnostic_plots/*.png        - fan charts and p50-vs-actual overlays

USAGE
-----
    python probabilistic_forecaster_with_regime.py \
...
# Grains with structural shift (test_mean / train_mean > 2.0 at any horizon)
SHIFT_GRAINS = [
    "Google Ads",
    "Meta Ads",
    "Organic/Direct",
...
def engineer_features(df):
    """Add cyclic month, trend, calendar, and regime_shift features."""
    df = df.copy()
    ...
    # Regime shift: 0 before 2026-02-01, 1 on or after
    df["regime_shift"] = (df["date"] >= pd.Timestamp(TRAIN_CUTOFF)).astype(int)
    return df


def build_feature_matrix(df, horizon, grain):
    ...
    For grains with a structural shift, a regime_shift_interaction feature
    (regime_shift * target_spend_Hd) is added to capture the regime's effect
    ...
    # Add regime_shift_interaction for grains with structural shift
    interaction_col = f"regime_shift_interaction_{horizon}d"
    if grain in SHIFT_GRAINS and spend_col in df.columns:
        df[interaction_col] = df["regime_shift"] * df[spend_col]
        base_cols.append(interaction_col)
...
def run_forecast(features_path, grain_col, grains, outdir, label):
    ...
    for grain in grains:
        ...
        for H in HORIZONS:
            X, feature_names, valid = build_feature_matrix(sub, H, grain)
...
def run(features_channel_path, features_campaign_type_path, outdir):
    ...
    print("Stage 4 – Probabilistic Forecasting with Regime Detection")
    print("=" * 55)
    ...
    print("\nStage 4 with regime detection complete.")
